...
The risk neutral gambler with a stake of money enters a game with the idea 
of betting until s/he's either reaches a goal N or runs out of money.  With a 
stake x and wager u, the resulting stake is either x+u with probability p or 
x-u with probability q (where p + q <= 1.) The wager must be smaller than the 
stake or any maximum wager established for the game.  To encourage risk 
taking, the future value of money may be discounted by a factor a <= 1.  
...
V(k,x) is the expected value of the game after the kth wager and with a stake 
x. If the gambler reaches the goal of winning a stake N at k then the value of
the game is V(k,N) = N. Or if the gambler loses everything, then V(k,0) = 0. 
Otherwise, for x < N, the Bellman equation for optimality provides the 
recursion

     V(k-1,x) = a max_u  [ p V(k,x+u) + q V(k,x-u) ]

where a is the discount factor for future values. The maximization is over the 
set of possible bets ranging from 0 to the minimum of x, N-x, or the bet limit 
B. Note that the state space and set of control actions are finite.
...
minimizing sum{x in 0..N} V[x]  subject to 
