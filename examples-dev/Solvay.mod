...
table chrt1 {s in SPECIES} OUT "JSON" "ColumnChart" :
    s~Species,
    sum{r in REACTIONS} max(0,(mw[s]*stoich[s,r]*x[r]))~Generated,
    sum{r in REACTIONS} -max(0,-(mw[s]*stoich[s,r]*x[r]))~Consumed;
table chrt2 {s in RAW_MATERIALS} OUT "JSON" "PieChart" : s, -mw[s]*n[s]~Consumed;
table chrt3 {s in SPECIES diff RAW_MATERIALS} OUT "JSON" "PieChart" : 
    s, mw[s]*n[s]~Generated;

table tab1 {s in RAW_MATERIALS} OUT "JSON" "Table" : 
    name[s]~Raw_Material, s~Formula, mw[s]~MolWt, 
    sum{r in REACTIONS} max(0,(mw[s]*stoich[s,r]*x[r]))~Generated,
    sum{r in REACTIONS} max(0,-(mw[s]*stoich[s,r]*x[r]))~Consumed,
    mw[s]*n[s]~Net;

table tab1 {s in PRODUCTS} OUT "JSON" "Table" : 
    name[s]~Product, s~Formula, mw[s]~MolWt, 
    sum{r in REACTIONS} max(0,(mw[s]*stoich[s,r]*x[r]))~Generated,
    sum{r in REACTIONS} max(0,-(mw[s]*stoich[s,r]*x[r]))~Consumed,
    mw[s]*n[s]~Net;
    
table tab3 {s in BYPRODUCTS} OUT "JSON" "Table" : 
